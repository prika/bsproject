      ...
      >
        <div class="moreinfo row">
          ...
          <p class="col-12 col-lg-6">
            ...
            ><br />{{ variant.size.width }}x{{ variant.size.height }}x{{
              variant.size.depth
            }}
            {{ $t("units_cm") }} -
            <span class="color">{{ variant.size.total }}</span>
            {{ $t("units_total_m3") }} m<sup>3</sup>
          </p>
          <p class="col-12 col-lg-6">
            <span class="title">{{ $t("units_weight") }}</span
            ><br /><span class="color">{{ variant.weight }}</span>
            {{ $t("units_ton") }}
          </p>
          <a
    ...
    >
      <div class="row pagecontrols">
        <a
          @click="$router.go(-1)"
          href="javascript:void(0)"
          class="backlink col-3"
          ><arrowRightIcon />{{ $t("actions_back") }}</a
        >
        <a
          @click="showShareModal = true"
          class="shareLink col-3"
          href="javascript:void(0)"
          ><arrowRightIcon />{{ $t("actions_share") }}<shareIcon
        /></a>
        ...
        <a
          ...
          v-if="!productAdded"
          href="javascript:void(0)"
          class="cartLink col-6"
          ><arrowRightIcon />{{ $t("actions_addtocart") }} <cartIcon
        /></a>
        <a
          ...
          ><arrowRightIcon />{{ $t("actions_removefromcart") }} <cartIcon
        /></a>
      </div>
    </transition>
...
export default {
  ...
  created() {
    this.$http
      .get("https://dev5.incentea-mi.pt/bstone/mocks/products-variant-detail-mock.json")
      .then((response) => {
