		...
		<transition appear enter-active-class="animated slideInUp delay-1s">
			<div class="wrapper">
				<div class="masonry effect" :page="currentPage">
					<figure
						v-for="(image, index) in selectedThumbs"
						:key="image.id+index"
				...
				<a
					@click="loadMoreClick"
					v-if="currentPage < this.thumbs.length / this.itemsPerComponent"
...
<script>
	...
	export default {
		...
		methods: {
			...
			loadMoreClick() {
				let slice = this.thumbs.slice(
					this.currentPage * this.itemsPerComponent,
					(this.currentPage + 1) * this.itemsPerComponent
				);
		...
		.masonry {
			...
			.masonry-brick {
				...
				.masonry-img {
					...
					-webkit-transition: all 0.3s cubic-bezier(0.4, 1.03, 0.83, 0.56);
					-moz-transition: all 0.3s cubic-bezier(0.4, 1.03, 0.83, 0.56);
					-o-transition: all 0.3s cubic-bezier(0.4, 1.03, 0.83, 0.56);
					transition: all 0.3s cubic-bezier(0.4, 1.03, 0.83, 0.56);
