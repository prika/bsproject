<template>
	<div class="modalSearch">
		...
		<div class="row" :class="{hasSearchText}">
		...
		<transition
			...
			leave-active-class="animated fadeOutDownBig faster"
		>
			<div class="resultsList" v-if="hasSearchText" :class="{hasSearchText}">
				<div class="container col-12">
					<div v-for="section in sections" :key="section.id">
						<!-- v-if="section.length > 0" -->
						<p class="numberResults" v-if="section.results > 0">
							{{ section.title }}
							<span></span>
							{{ section.results }}
						</p>

						<ul v-if="section.results > 0">
							<li v-for="itemlist in section.list" :key="itemlist.id">
								<router-link :to="itemlist.link">
									<div class="img">
										<img width="190px" height="190px" :src="itemlist.src" :alt="itemlist.name" />
									</div>
									...
									<div class="text">
										...
										<p>
											{{itemlist.stock}}
											<span class="stock">{{ $t('in-stock')}}</span>
										</p>
									</div>
								</router-link>
							</li>
				...
				<!-- SEM RESULTADOS -->
				<div class="container col-12" v-if="hasSearchText && !hasResults">
...
<style lang="scss">
	...
	.resultsList {
		...
		background: #fff url(../../assets/images/shared/bg_search.svg) no-repeat -200px
			top;
